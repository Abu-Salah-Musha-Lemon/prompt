...
### `ProcessData` Subroutine Convert status to lowercase for uniform comparison
```vba
Sub ProcessData()
    ...
    For i = 4 To 26
        ...
        If InStr(nameStatus, ":") > 0 Then
            ...
            ' Update column A and B
            ws.Cells(i, 1).Value = namePart
            ws.Cells(i, 2).Value = statusPart
        Else
            statusPart = ws.Cells(i, 2).Value ' fallback if already separated
        End If

        ' Convert status to lowercase for uniform comparison
        statusLower = LCase(statusPart)

        ' Define row range from A to G
        Set rowRange = ws.Range(ws.Cells(i, 1), ws.Cells(i, 7))

        ' Apply color formatting
        If InStr(statusLower, "leave") > 0 Or _
           InStr(statusLower, "absent") > 0 Or _
           InStr(statusLower, "cl") > 0 Then
            rowRange.Interior.Color = RGB(255, 0, 0) ' Red
        ElseIf InStr(statusLower, "with") > 0 Then
            rowRange.Interior.Color = RGB(0, 255, 0) ' Green
        Else
            rowRange.Interior.Color = RGB(255, 255, 255) ' White
        End If
    Next i
